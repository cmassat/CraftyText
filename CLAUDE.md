...
CraftyText is a WYSIWYG markdown editor built on Electron + Vue 3. It supports CommonMark, GitHub Flavored Markdown, math (KaTeX), Mermaid diagrams, PlantUML, and multiple editing modes (focus, typewriter, source-code).

- **Version**: see `package.json`
- **License**: MIT
- **Repository**: https://github.com/cmassat/craftytext
...
<repo-root>/
  package.json              Workspace orchestrator — every CI-facing script
                            ...
                            craftytext ...`. CI invocations are unchanged.
  ...
  packages/
    ...
    muyajs/                 Legacy markdown editor engine
                            (name: "@craftytext/muyajs"). Primarily JS + DOM,
    ...
    muya/                   TypeScript rewrite of muya
      ...
      e2e/                  muya-e2e — Playwright suite. CI runs Chromium
                            ...
                            specs.
    website/                craftytext-website (Next.js + React). Standalone
...
desktop-specific script to `packages/desktop` via `pnpm --filter craftytext`,
...
pnpm --filter craftytext-website dev      # Next.js dev server
pnpm --filter craftytext-website build    # production website build
```
...
alias / `@craftytext/muyajs` workspace dep.
...
Muya  (packages/muyajs/)            ← workspace package @craftytext/muyajs
...
`packages/website/content/docs/dev/` contains the deeper developer documentation referenced by this guide. The website package publishes these files under `/docs/dev/overview`.
...
- **Path aliases** (defined in `packages/desktop/electron.vite.config.ts`, mirrored in `vitest.config.ts` and `tsconfig.base.json`):
  ...
  - `muya` → `../muyajs` (i.e. `packages/muyajs`). Renderer-side imports therefore look like `muya/lib/...` (the alias) — the workspace dep `@craftytext/muyajs` is declared in `packages/desktop/package.json` so module resolution stays inside the workspace.
