...
The most simplest way is to debug using the `Debug CraftyText` configuration. You can set breakpoints and use the `debugger` statement.
...
After the successful launch of CraftyText, press `Ctrl+C` on both `node-profiler` instances. The tools created two files named `main.cpuprofile` and `renderer.cpuprofile`. You can now analyse these files via _Chrome Developer Tools_ or _Visual Studio Code_.
